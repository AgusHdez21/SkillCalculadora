...
const LaunchRequestHandler = {
    ...
    handle(handlerInput) {
        const speakOutput = 'Hola Agus, bienvenido a calculadora uthh';
...
const handleSumaInput = {
  canHandle(handlerInput){
      return Alexa.getRequestType(handlerInput.requestEnvelope) === 'IntentRequest'
          && Alexa.getIntentName(handlerInput.requestEnvelope) === 'SumaInput'
  },
  handle(handlerInput){
    ...
    const speakOutput = ` El resultado de la suma es ${resultado} `
    return handlerInput.responseBuilder
      .speak(speakOutput)
      .getResponse()
  }
...
const PowerIntentHandler = {
  canHandle(handlerInput) {
    return Alexa.getRequestType(handlerInput.requestEnvelope) === 'IntentRequest'
        && Alexa.getIntentName(handlerInput.requestEnvelope) === 'PotenciaInput';
  },
  handle(handlerInput) {
    const baseValue = handlerInput.requestEnvelope.request.intent.slots.base.value;
    const exponentValue = handlerInput.requestEnvelope.request.intent.slots.exponent.value;
    ...
    const resultado = Math.pow(base, exponent);
    const speakOutput = `El resultado de ${base} elevado a la potencia de ${exponent} es ${resultado}`;
...
// FuncionRaiz
const SquareRootIntentHandler = {
  canHandle(handlerInput) {
    return Alexa.getRequestType(handlerInput.requestEnvelope) === 'IntentRequest'
        && Alexa.getIntentName(handlerInput.requestEnvelope) === 'RaizInput';
  },
  handle(handlerInput) {
    const numberValue = handlerInput.requestEnvelope.request.intent.slots.number.value;
    
    const numero = Number(numberValue);
    
    if (isNaN(numero)) {
      return handlerInput.responseBuilder
        .speak("Lo siento, no pude entender el número. Por favor, intente de nuevo")
        .getResponse();
    }
    
    if (numero< 0) {
      return handlerInput.responseBuilder
        .speak("Lo siento, no puedo calcular la raíz cuadrada de un número negativo. Por favor, intente de nuevo con un número positivo")
        .getResponse();
    }
